...
export default class BoomBaseSubCylinders {
    ...
    testStructure() {
        ...
        let cylGeo = new THREE.CylinderGeometry(0.5, 0.5, 2)
        let box = new THREE.Mesh(boxGeo, this.material)
        let cyl = new THREE.Mesh(cylGeo, this.material)
        box.position.x = 2
        cyl.position.x = 4

        this.scene.add(box, cyl)
    ...
    setHorCylinders(height) {
        ...
        let squareLength2 = Math.cos(deg2rad(90 - this.xRotAngle)) * height * 2
        ...
        squareCyl1.position.y = (Math.sin(deg2rad(90 - (this.zRotAngle))) * height / 2) - yOffsetAdjustment
        squareCyl1.position.z = Math.sin(deg2rad(this.xRotAngle)) * height
        ...
        squareCyl3.position.y = (Math.sin(deg2rad(90 - (this.zRotAngle))) * height / 2) - yOffsetAdjustment
        ...
        squareCyl4.position.y = (Math.sin(deg2rad(90 - (this.zRotAngle))) * height / 2) - yOffsetAdjustment
